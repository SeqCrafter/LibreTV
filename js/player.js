...
let currentDanmukuUrl = null; // 记录当前弹幕URL
const isWebkit = typeof window.webkitConvertPointFromNodeToPage === "function";
Artplayer.FULLSCREEN_WEB_IN_BODY = true;

// 转换弹幕数据格式
function convertDanmakuData(apiData) {
  ...
  return apiData.danmuku.map(([time, mode, color, fontSize, text]) => ({
    ...
    time: time,
    color: color || "#FFFFFF",
    mode: mode === "right" ? 0 : mode === "top" ? 1 : mode === "bottom" ? 2 : 0,
    fontSize: fontSize ? parseInt(fontSize) : 25,
  }));
}

// 创建弹幕加载函数
function createDanmakuLoader(danmukuUrl) {
  return () => {
...
// 构建弹幕URL的辅助函数
function buildDanmakuUrl(doubanId, episodeNumber) {
...
function initializePageContent() {
  ...
  try {
    if (episodesList) {
      ...
      // 否则从localStorage获取
      currentEpisodes = JSON.parse(
        localStorage.getItem("currentEpisodes") || "[]"
      );
    }
    ...
    if (
      index < 0 ||
      (currentEpisodes.length > 0 && index >= currentEpisodes.length)
    ) {
      // 如果索引太大，则使用最大有效索引
  ...
  if (videoUrl) {
    // 构建弹幕URL
    const doubanId = urlParams.get("douban_id");
    if (doubanId) {
      // 集数从1开始（currentEpisodeIndex是0-based）
      const episodeNumber = currentEpisodeIndex + 1;
      currentDanmukuUrl = buildDanmakuUrl(doubanId, episodeNumber);
      console.log("Constructed danmaku URL:", currentDanmukuUrl);
    } else {
      currentDanmukuUrl = null;
    }

    initPlayer(videoUrl, currentDanmukuUrl);
  } else {
    showError("无效的视频链接");
  }
...
// 初始化播放器
function initPlayer(videoUrl, danmukuUrl) {
  if (!videoUrl) {
    return;
  }

  // 更新全局弹幕URL
  currentDanmukuUrl = danmukuUrl;
  ...
  const danmukuOptions = {
    danmuku: createDanmakuLoader(danmukuUrl), // 使用全局函数创建弹幕加载器
  ...
  art.on("video:error", function (error) {
    ...
    const loadingElements = document.querySelectorAll(
      "#player-loading, .player-loading-container"
    );
...
function updateEpisodeInfo() {
  if (currentEpisodes.length > 0) {
    document.getElementById("episodeInfo").textContent = `第 ${
...
function renderEpisodes() {
  ...
  if (!currentEpisodes || currentEpisodes.length === 0) {
    episodesList.innerHTML =
      '<div class="col-span-full text-center text-gray-400 py-8">没有可用的集数</div>';
    return;
  }
...
function playEpisode(index) {
  ...
  // 构建新的弹幕URL
  const doubanId = currentUrl.searchParams.get("douban_id");
  let newDanmukuUrl = null;
  if (doubanId) {
    const episodeNumber = index + 1; // 集数从1开始
    newDanmukuUrl = buildDanmakuUrl(doubanId, episodeNumber);
    console.log("切换集数，新弹幕URL:", newDanmukuUrl);
  }

  if (isWebkit) {
    // WebKit需要重新初始化播放器
    initPlayer(url, newDanmukuUrl);
  } else {
    // 非WebKit只切换视频URL
    art.switch = url;
...
function saveToHistory() {
  ...
  const douban_id = urlParams.get("douban_id") || ""; // Get douban_id from URL
  ...
  if (sourceName && id_from_params) {
    ...
    show_identifier_for_video_info =
  ...
  const videoInfo = {
    ...
    url: `player.html?url=${encodeURIComponent(
      currentVideoUrl
    )}&title=${encodeURIComponent(
      currentVideoTitle
    )}&source=${encodeURIComponent(
      sourceName
    )}&source_code=${encodeURIComponent(sourceCode)}&id=${encodeURIComponent(
      id_from_params || ""
    )}&index=${currentEpisodeIndex}&position=${Math.floor(
      currentPosition || 0
    )}${douban_id ? `&douban_id=${encodeURIComponent(douban_id)}` : ""}`,
    episodeIndex: currentEpisodeIndex,
    sourceName: sourceName,
    vod_id: id_from_params || "", // Store the ID from params as vod_id in history item
    sourceCode: sourceCode,
    douban_id: douban_id, // Save douban_id
    showIdentifier: show_identifier_for_video_info, // Identifier for the show/series
    timestamp: Date.now(),
    playbackPosition: currentPosition,
    duration: videoDuration,
    episodes:
      currentEpisodes && currentEpisodes.length > 0 ? [...currentEpisodes] : [],
  };
  ...
  try {
    ...
    if (existingIndex !== -1) {
      ...
      existingItem.douban_id =
        videoInfo.douban_id || existingItem.douban_id || ""; // Update douban_id
      ...
      // 更新播放进度信息
      existingItem.playbackPosition =
        videoInfo.playbackPosition > 10
      ...
      if (videoInfo.episodes && videoInfo.episodes.length > 0) {
        if (
          ...
          // Basic check for content change
...
function showPositionRestoreHint(position) {
  ...
  if (playerContainer) {
    // Check if playerContainer exists
...
function closeEmbeddedPlayer() {
  try {
    if (window.self !== window.top) {
      ...
      if (
        window.parent &&
        typeof window.parent.closeVideoPlayer === "function"
      ) {
...
async function testVideoSourceSpeed(sourceKey, vodId) {
  try {
    ...
    const response = await fetch(
      `/api/detail?id=${encodeURIComponent(vodId)}${apiParams}${cacheBuster}`,
      {
        method: "GET",
        cache: "no-cache",
      }
...
async function showSwitchResourceModal() {
  ...
  const sortedResults = Object.entries(allResults).sort(
    ([keyA, resultA], [keyB, resultB]) => {
      // 当前播放的源放在最前面
      const isCurrentA =
        String(keyA) === String(currentSourceCode) &&
        String(resultA.vod_id) === String(currentVideoId);
      const isCurrentB =
        String(keyB) === String(currentSourceCode) &&
        String(resultB.vod_id) === String(currentVideoId);
            ...
            <div class="relative group ${
              isCurrentSource
                ...
                : "cursor-pointer hover:scale-105 transition-transform"
                 ...
                 ${
                   !isCurrentSource
                     ? `onclick="switchToResource('${sourceKey}', '${result.vod_id}')"`
                     : ""
                 }>
                ...
                <div class="mt-2">
                    ...
                    <div class="text-[10px] text-gray-500 mt-1">
                        ${
                          speedResult.episodes
                            ? `${speedResult.episodes}集`
                            : ""
                        }
                    </div>
                </div>
...
async function switchToResource(sourceKey, vodId) {
  ...
  try {
    ...
    // 构建播放页面URL
    const watchUrl = `player.html?id=${vodId}&source=${sourceKey}&url=${encodeURIComponent(
      targetUrl
    )}&index=${targetIndex}&title=${encodeURIComponent(currentVideoTitle)}`;
