...
function initDouban() {
  ...
  if (doubanToggle) {
    // 如果从未设置过，默认为 true
    const storedValue = localStorage.getItem("doubanEnabled");
    const isEnabled = storedValue === null ? true : storedValue === "true";
  ...
  // 初始加载热门内容
  const storedEnabled = localStorage.getItem("doubanEnabled");
  if (storedEnabled === null || storedEnabled === "true") {
...
function renderDoubanTags(tags) {
  ...
  // 确定当前应该使用的标签列表
  const currentTags =
    doubanMovieTvCurrentSwitch === "movie" ? movieTags : tvTags;
  ...
  manageBtn.innerHTML =
    '<span class="flex items-center"><svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>管理标签</span>';
  ...
  currentTags.forEach((tag) => {
    ...
    // 设置样式
    let btnClass =
      "py-1.5 px-3.5 rounded text-sm font-medium transition-all duration-300 border ";
...
function renderDoubanCards(data, container) {
  ...
  if (!data.subjects || data.subjects.length === 0) {
    ...
    data.subjects.forEach((item) => {
      ...
      card.className =
        "bg-[#111] hover:bg-[#222] transition-all duration-300 rounded-lg overflow-hidden flex flex-col transform hover:scale-105 shadow-md hover:shadow-lg";
...
function showTagManageModal() {
  ...
  modal.innerHTML = `
        <div class="bg-[#191919] rounded-lg p-6 max-w-md w-full max-h-[90vh] overflow-y-auto relative">
            ...
            <h3 class="text-xl font-bold text-white mb-4">标签管理 (${
              isMovie ? "电影" : "电视剧"
            })</h3>
